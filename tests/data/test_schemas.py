...
def test_presence_validation_fails_if_returned_attribute_that_never_should_be_returned(
    ...
    issues = user_schema.validate(user_data_server, AttrValuePresenceConfig("RESPONSE"))
...
def test_presence_validation_fails_on_attr_not_requested_by_exclusion(user_schema):
    ...
    issues = user_schema.validate(
        data, AttrValuePresenceConfig("RESPONSE", attr_reps=[AttrRep(attr="name")], include=False)
    )
...
def test_presence_validation_fails_on_attr_not_requested_by_inclusion(user_schema):
    ...
    issues = user_schema.validate(
        data, AttrValuePresenceConfig("RESPONSE", attr_reps=[AttrRep(attr="name")], include=True)
    )
...
def test_presence_validation_fails_if_not_provided_attribute_that_always_should_be_returned(
    ...
    issues = user_schema.validate({}, AttrValuePresenceConfig("RESPONSE"))
...
def test_presence_validation_fails_if_not_provided_requested_required_attribute(user_schema):
    ...
    issues = user_schema.validate(
        data,
        AttrValuePresenceConfig("RESPONSE", attr_reps=[AttrRep(attr="username")], include=True),
    )
...
def test_resource_schema_cls_can_be_created_from_schema_rep(user_schema_rep):
    user_schema_rep["id"] = "some:custom:schema"
    user_schema_rep["name"] = "CustomUserSchema"
    ...
    assert schema.name == "CustomUserSchema"
    assert schema.endpoint == "/CustomUsers"
    assert schema.schema == "some:custom:schema"
    assert len(list(schema.attrs)) == 19
